...
package neoart.flod.trackers {
  ...
  public final class HMPlayer extends AmigaPlayer {
    ...
    override protected function loader(stream:ByteArray, extra:ByteArray):void {
      var count:int, empty:HMSample, higher:int, i:int, id:String, j:int, mupp:int, pos:int, row:BaseRow, sample:HMSample, size:int, value:int;

      if (stream.length < 2106) return;

      stream.position = 1080;
      if (stream.readUTFBytes(4) != "FEST") return;
    ...
    private function process():void {
      var chan:AmigaChannel, pattern:int, row:BaseRow, sample:HMSample, value:int, voice:HMVoice = voices[0];
    ...
    private function effects(voice:HMVoice):void {
      var i:int, len:int, period:int = voice.period & 0x0fff, slide:int, value:int;
